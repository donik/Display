...
open class ListView: ASDisplayNode, UIScrollViewAccessibilityDelegate, UIGestureRecognizerDelegate {
    ...
    private var itemReorderingTimer: GDKSwiftSignalKit.Timer?
    ...
    private func checkItemReordering(force: Bool = false) {
        ...
        if !force {
            self.itemReorderingTimer = GDKSwiftSignalKit.Timer(timeout: 0.025, repeat: false, completion: { [weak self] in

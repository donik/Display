...
public final class ListViewReorderingGestureRecognizer: UIGestureRecognizer {
    ...
    private var longTapTimer: GDKSwiftSignalKit.Timer?
    private var longPressTimer: GDKSwiftSignalKit.Timer?
    ...
    private func startLongTapTimer() {
        self.longTapTimer?.invalidate()
        let longTapTimer = GDKSwiftSignalKit.Timer(timeout: 0.25, repeat: false, completion: { [weak self] in
    ...
    private func startLongPressTimer() {
        self.longPressTimer?.invalidate()
        let longPressTimer = GDKSwiftSignalKit.Timer(timeout: 0.6, repeat: false, completion: { [weak self] in
